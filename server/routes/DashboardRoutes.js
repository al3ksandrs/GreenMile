    ...
    async #getLKI() {
        this.#app.get("/lki", async (req,res) => {
            try {
                ...
                await fetch("https://api.luchtmeetnet.nl/open_api/lki?station_number=NL49017&start=" + new Date(Date.now() - 3600001).toISOString() + "&end=" + new Date(Date.now()).toISOString(), requestOptions)
